...
-- TRAJECTORY_BUILDER_2D.num_accumulated_range_data = 1
-- TRAJECTORY_BUILDER_2D.submaps.num_range_data = 30 --40
-- TRAJECTORY_BUILDER_2D.min_range = 1
-- TRAJECTORY_BUILDER_2D.max_range = 60
-- TRAJECTORY_BUILDER_2D.min_z = 0

-- TRAJECTORY_BUILDER_2D.voxel_filter_size = 0.02  -- modify 0.015
-- TRAJECTORY_BUILDER_2D.loop_closure_adaptive_voxel_filter.max_length = 0.015 --0.015
...
-- TRAJECTORY_BUILDER_2D.real_time_correlative_scan_matcher.translation_delta_cost_weight = 0.1 --1
...
-- TRAJECTORY_BUILDER_2D.adaptive_voxel_filter.max_length = 0.5 --0.015
-- TRAJECTORY_BUILDER_2D.ceres_scan_matcher.ceres_solver_options.max_num_iterations = 30 --50
-- TRAJECTORY_BUILDER_2D.adaptive_voxel_filter.max_range = 12.  -- yuan 30
-- TRAJECTORY_BUILDER_2D.motion_filter.max_time_seconds = 0.25 --1
...
-- 轨迹构建器优化配置
TRAJECTORY_BUILDER_2D.num_accumulated_range_data = 1
TRAJECTORY_BUILDER_2D.submaps.num_range_data = 30 -- 保持较小以支持快速重定位
TRAJECTORY_BUILDER_2D.min_range = 1
TRAJECTORY_BUILDER_2D.max_range = 60
TRAJECTORY_BUILDER_2D.min_z = 0
...
TRAJECTORY_BUILDER_2D.use_imu_data = true
TRAJECTORY_BUILDER_2D.use_online_correlative_scan_matching = true
TRAJECTORY_BUILDER_2D.real_time_correlative_scan_matcher.linear_search_window = 0.2  -- 增加搜索窗口支持重定位
...
-- 优化频率控制
POSE_GRAPH.optimize_every_n_nodes = 0  -- 禁用自动优化，手动控制
POSE_GRAPH.optimization_problem.huber_scale = 1e2
...
-- 匹配阈值优化
POSE_GRAPH.constraint_builder.min_score = 0.3  -- 略微降低近距离匹配阈值
POSE_GRAPH.constraint_builder.global_localization_min_score = 0.6 -- 降低全局重定位阈值
...
POSE_GRAPH.constraint_builder.fast_correlative_scan_matcher.linear_search_window = 20.  -- 增加线性搜索窗口
